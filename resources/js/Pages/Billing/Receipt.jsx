...
export default function Receipt({ invoice }) {
  ...
  return (
    <>
      ...
      {/* Print styles — Tailwind's print: modifier doesn't cover display:none reliably */}
      <style>{`
        @media print {
      ...
      <div className="min-h-screen bg-gray-100 py-10 px-4 print:bg-white print:p-0 print:m-0">
        ...
        <div className="no-print max-w-2xl mx-auto mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/*
             * Download PDF — plain <a> tag, NOT Inertia <Link>.
             * This triggers a file download response from DomPDF.
             * If the Inertia Link component intercepted it, the browser
             * would try to render the binary PDF as a page and fail.
             */}
            <a
              href={`/billing/invoices/${invoice.id}/download`}
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium border border-gray-200 bg-white hover:bg-gray-50 text-gray-700 rounded-lg transition-colors"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="max-w-2xl mx-auto bg-white shadow-md rounded-2xl p-8 print:shadow-none print:rounded-none print:p-6">
          ...
          <div className="grid grid-cols-2 gap-6 mb-8">
            <div>
              ...
              {invoice.patient?.display_mobile && (
                <p className="text-xs text-gray-500 mt-0.5">{invoice.patient.display_mobile}</p>
              )}
              {invoice.patient?.email && (
                <p className="text-xs text-gray-500">{invoice.patient.email}</p>
              )}
              {invoice.patient?.address && (
                <p className="text-xs text-gray-500 mt-0.5">{invoice.patient.address}</p>
              )}
            </div>
            {invoice.appointment && (
              <div>
          ...
          <table className="w-full text-sm mb-6">
              ...
              <tr className="border-b border-gray-50">
                <td className="py-3">
                  <p className="font-medium text-gray-900">
                    {invoice.treatment?.procedure_display
            ...
            <tfoot>
              <tr>
                ...
                <td className="pt-3 text-right text-sm text-gray-700 font-medium">
                  {peso(invoice.subtotal)}
                </td>
              </tr>
                ...
                <tr>
                  <td className="pt-1 text-sm text-green-600">
                    ...
                    {invoice.discount_reason && (
                      <span className="text-xs text-gray-400"> — {invoice.discount_reason}</span>
                    )}
                  </td>
                  <td className="pt-1 text-right text-sm text-green-600 font-medium">
                    − {peso(invoice.discount_amount)}
                  </td>
                </tr>
              )}
